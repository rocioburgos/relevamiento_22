...
@Component({
  ...
  templateUrl: './cosasfeas.page.html',
  styleUrls: ['./cosasfeas.page.scss'],
})
export class CosasfeasPage implements OnInit {
 ...
 /* titulo='Cosas feas'; 
  ...
  tomarFoto(){ 
    ...
    localStorage.setItem('tipoAsubir', JSON.stringify({tipo:'feas'})); 
    this.router.navigate(['upload']);
  } 
  
  obtenerImagenes(){  
    //Traigo productos 
    this.imagenSrv.getImagenes('feas').subscribe( data => {
      this.contact =  data; 
      console.log(data)
